...
class Theme_My_Login_MS_Signup extends Theme_My_Login_Abstract {
	...
	public function tml_display_register( &$template ) {
		...
		if ( $active_signup == "none" ) {
			...
			printf( __( 'You must first <a href="%s">Авторизація</a>, and then you can create a new site.', 'theme-my-login' ), wp_login_url( Theme_My_Login_Common::get_current_url() ) );
		} else {
			...
			switch ( $stage ) {
				...
				case 'gimmeanotherblog':
					...
					<p>
						<?php printf( __( '<a href="http://%1$s">http://%2$s</a> is your new site.  <a href="%3$s">Авторизація</a> as &#8220;%4$s&#8221; using your existing password.', 'theme-my-login' ), $domain.$path, $domain.$path, "http://" . $domain.$path . "wp-login.php", $current_user->user_login ) ?>
					</p>
					<?php
					do_action( 'signup_finished' );
					break;
