...
class Theme_My_Login extends Theme_My_Login_Abstract {
	...
	public static function default_pages() {
		return apply_filters( 'tml_default_pages', array(
			'login'        => __( 'Авторизація'        , 'theme-my-login' ),
			...
			switch ( $this->request_action ) {
				...
				default:
					...
					if		( isset( $_GET['loggedout'] ) && true == $_GET['loggedout'] )
						$this->errors->add( 'loggedout', __( 'Тепер ви вийшли з системи.', 'theme-my-login' ), 'message' );
					...
					elseif	( $reauth )
						$this->errors->add( 'reauth', __( 'Авторизуйтесь для продовження.', 'theme-my-login' ), 'message' );
...
<?php
	...
	public static function retrieve_password() {
		...
		if ( empty( $_POST['user_login'] ) ) {
			$errors->add( 'empty_username', __( '<strong>ERROR</strong>: Enter a ПІБ читача українською мовою (повністю) address.', 'theme-my-login' ) );
		...
		if ( ! $user_data ) {
			$errors->add( 'invalidcombo', __( '<strong>ERROR</strong>: Invalid ПІБ читача українською мовою (повністю).', 'theme-my-login' ) );
			return $errors;
		}
