...
class Theme_My_Login_User_Moderation extends Theme_My_Login_Abstract {
	...
	public function action_messages( &$theme_my_login ) {
		if ( isset( $_GET['pending'] ) ) {
			switch ( $_GET['pending'] ) {
				case 'activation' :
					$theme_my_login->errors->add( 'pending_activation', __( 'Your registration was successful but you must now confirm your email address before you can Авторизація. Please check your email and click on the link provided.', 'theme-my-login' ), 'message' );
					break;
				case 'approval' :
					$theme_my_login->errors->add( 'pending_approval', __( 'Your registration was successful but you must now be approved by an administrator before you can Авторизація. You will be notified by e-mail once your account has been reviewed.', 'theme-my-login' ), 'message' );
					break;
			}
		}
			...
			switch ( $_GET['activation'] ) {
				case 'complete' :
					if ( class_exists( 'Theme_My_Login_Custom_Passwords' ) )
						$theme_my_login->errors->add( 'activation_complete', __( 'Your account has been activated. You may now Авторизація.', 'theme-my-login' ), 'message' );
					else
