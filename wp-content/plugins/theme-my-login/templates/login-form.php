...
<div class="tml tml-login" id="theme-my-login<?php $template->the_instance(); ?>">
	...
	<form name="loginform" id="loginform<?php $template->the_instance(); ?>" action="<?php $template->the_action_url( 'login', 'login_post' ); ?>" method="post">
		<p class="tml-user-login-wrap">
			<label for="user_login<?php $template->the_instance(); ?>"><?php
				if ( 'username' == $theme_my_login->get_option( 'login_type' ) ) {
					...
					_e( 'ПІБ читача українською мовою (повністю)', 'theme-my-login' );
				}
			?></label>
		...
		<p class="tml-user-pass-wrap">
			<label for="user_pass<?php $template->the_instance(); ?>"><?php _e( 'Штрих-код', 'theme-my-login' ); ?></label>
		...
		<div class="tml-rememberme-submit-wrap">
			<p class="tml-rememberme-wrap">
				...
				<label for="rememberme<?php $template->the_instance(); ?>"><?php esc_attr_e( 'Запам\'ятати мене', 'theme-my-login' ); ?></label>
			</p>

			<p class="tml-submit-wrap">
				<input type="submit" name="wp-submit" id="wp-submit<?php $template->the_instance(); ?>" value="<?php esc_attr_e( 'Авторизація', 'theme-my-login' ); ?>" />
